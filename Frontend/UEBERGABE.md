...
**Anfragenkorb (seit 10.09.2026, Details in §4):** Das Altsystem führt zwei Betriebsarten
desselben Weges. Die Brücke erkennt beide:
- `GET /api/kasse/vorschau` liefert `art: "bestellung" | "anfrage" | null` (kein Knopf),
  `uebersicht.absendeknopfText` wörtlich vom Altsystem („Anfrage abschicken" bzw.
  „Bestellung abschicken"), `uebersicht.korb` („Warenkorb" | „Anfragenkorb") und
  `uebersicht.items` auch für den Anfragenkorb (Preis `"auf Anfrage"` bleibt Zeichenkette,
  `preisNum: null`; Positionen ohne Namen haben `name: null`, dafür `beschreibung`).
  Bei `art: "anfrage"` gibt es keine Hürde „Unbekannte Zahlungsart"; Ogone bleibt gesperrt.
- `POST /api/kasse/bestellen` sendet **das Feld, das die Übersichtsseite anbietet**
  (`anfrage_abschicken` oder `bestellung_abschicken`, nie ein erfundenes) und antwortet mit
  `art`. `JA-BESTELLEN` und `BESTELLUNG_GESPERRT` gelten für beide Wege.
- `GET /api/kasse/formular` liefert beim Anfragenkorb `optionen.zahlungsart.optionen: []`
  (kein Fehler) und `optionen.zahlungsart.versteckt` — das versteckte Feld des Altsystems
  (beobachtet `"RechnungPayment"`). Die **Versandart bleibt auch dort wählbar**
  (DHL/pauschal/DPD/Spedition); nur die Zahlungsart fehlt.
- `GET /api/cart` (und `warenkorb` in Formular/Vorschau) trägt `modus: "kauf" | "anfrage" |
  "gemischt" | null` und je Position `modus`. Ein gemischter Korb wird im Altsystem zum
  Anfragenkorb — siehe §4.

Die Regeln stehen an einer Stelle (`ermittleHuerden()` in `lib/bruecke.mjs`) und werden ohne
Netz von `node pruefe-anfrage.mjs` gegen die Rohseiten unter `bridge-demo/fixtures/` geprüft.
...
### 4.1 Der Anfrage-Weg (belegt 10.09.2026, Rohseiten in `bridge-demo/fixtures/`)

Anfrageartikel („in den Anfragenkorb", z. B. `/logomatten/6300201-logomatte-a`, Artikel 569,
`modus: "anfrage"` in `/api/produkt`) laufen denselben Weg `/warenkorb → /adresse → /bestellen`,
aber das Altsystem schaltet ihn um:

| | Kauf-Warenkorb | Anfragenkorb |
|---|---|---|
| `/warenkorb` | Versandart + Zahlungsart-Radios | Versandart wählbar; Zahlungsart **nur als verstecktes Feld** `<input type="hidden" name='zahlungsart' value="RechnungPayment">` |
| Mini-Warenkorb | Betrag | Betrag wird trotzdem angezeigt (147,48 EUR bei 1 × 569 in 90 × 120 cm) — das Altsystem rechnet intern, zeigt in der Tabelle aber „auf Anfrage" |
| `/bestellen` Überschrift | Warenkorb | **Anfragenkorb** |
| `/bestellen` Tabelle | Positionen, Zwischensumme, Versand, USt, Gesamt, „Zahlungsart: …" | Positionen mit Preis „auf Anfrage"; **keine Summen, keine Zahlungsart** |
| Absende-Knopf | `bestellung_abschicken` = „Bestellung abschicken" | **`anfrage_abschicken` = „Anfrage abschicken"** |
| `POST /bestellen` | 302 → `/danke`, Auftrag im Admin | **nicht live geprüft** — der Anfrage-Abschluss wurde nie ausgelöst |

**Gemischter Korb (Kauf + Anfrage):** in beiden Reihenfolgen live geprüft (nur bis zur Vorschau).
Ergebnis ist in beiden Fällen derselbe Anfragenkorb: Überschrift „Anfragenkorb", Knopf
`anfrage_abschicken`, keine Zahlungsart-Radios (verstecktes `RechnungPayment`), keine Summen.
Die Kaufposition steht mit ihrem Preis (25,17 €) in der Tabelle, **wird aber nicht bestellt,
sondern mit angefragt**. `POST /warenkorb` mit `zahlungsart` ändert daran nichts. Der
Mini-Warenkorb addiert beides (147,48 + 29,95 = „177,43 EUR"). Die Brücke sendet den
vorhandenen Knopf (`art: "anfrage"`, `warenkorb.modus: "gemischt"`); **das Frontend muss dem
Besucher sagen, dass mit einem Anfrageartikel im Korb nichts gekauft wird** — oder das Mischen
verhindern. Ob das Altsystem die Kaufposition in der Anfrage überhaupt übernimmt, ist offen
(nie abgeschickt).
...
11. **Ein einziger Anfrageartikel macht den ganzen Korb zum Anfragenkorb** — Kaufartikel darin
    werden nicht bestellt, sondern mit angefragt (§4.1). Die Zahlungsart führt der Anfragenkorb
    als verstecktes Feld `RechnungPayment`, der Mini-Warenkorb zeigt trotzdem einen Betrag.
...
# Anfrage-Weg der Brücke prüfen (ohne Netz, gegen fixtures/)
