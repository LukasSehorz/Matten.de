...
28. **Formular-Feinschliff** (17.09.2026, zweite Mail Fuchsius 12:08): `Rand` steht vor `Form`; die Sonderfarben stehen einzeilig als „Anzahl gewünschter Sonderfarben: 0= keine" mit schmalem Zahlenfeld; Felder und Abstände sind flacher (`#order-form` in `net-neu.css`), damit die Artikelbeschreibung weiter nach oben rückt; „inkl. MWSt." steht hinter dem Preis statt darunter; die Endpreis-Zeile bleibt einzeilig; der Knopf heißt **„Angebot anfordern"** statt „Make an offer"; Formular und Farbfeld-Block sind gleich breit (540 px bei 1280 px Fensterbreite).
29. **„Standard" / „Sonderanfertigung" neben dem Preis** (WhatsApp-Notiz Fuchsius 17.09.2026, 15:38; Farben aus der Nachricht vom 14.09.): grün, solange eine Fixgröße getroffen wird und weder Sonderform noch Sonderfarbe gewählt sind — sonst rot. Entscheidung über `groessenArtZeigen()` anhand derselben Prüfung, die den Zielartikel bestimmt (`weg().freieMasse`, `zuschlaege()`).
...
8. **Preis mehrerer Sonderfarben — beantwortet (17.09.2026).** Herr Fuchsius bestätigt: **68,- € netto je Sonderfarbe**, unabhängig von der Stückzahl, einmal je Auftrag (zwei Farben 136,- €, drei 204,- €) — weil die Farben auftragsbezogen speziell gemischt werden und sich nicht lagern lassen. Genau so rechnet `preisformel.js`. Der **Einkaufsaufschlag liegt bei 54,- € je Sonderfarbe** („momentan"), nicht bei den 50,- € der Excel-Mappe; er steht deshalb als `aufschlagSonderfarbeEK: 54` bei den Stammdaten in `seite-produkt.js` — die Mappe bleibt unangetastet, weil `pruefe-preisformel.mjs` gegen sie vergleicht. Ursprünglicher Stand der Rückfrage: Die Excel-Vorlage kennt in P3 nur „Sonderfarbe ja/nein" mit 68,- € netto (VK) bzw. 50,- € (EK), ausdrücklich einmal je Auftrag. Seit dem Kundenwunsch nach einer Anzahl rechnet `preisformel.js` **68,- € je Sonderfarbe**, weiterhin nur einmal je Auftrag (`sonderfarbenAnzahl`; zwei Farben = 136,- € netto, unabhängig von der Stückzahl). Dass sich der Betrag je Farbe vervielfacht, ist eine **Annahme** — der Entwurf der Rückfrage liegt in `Frontend/_arbeit/FRAGE-SONDERFARBEN.md`. Bleibt es bei 68,- € pauschal, genügt es, in `preisformel.js` den Faktor `sonderfarbenAnzahl` bei `aufschlagVK`/`aufschlagEK` wieder auf 1 zu setzen.
