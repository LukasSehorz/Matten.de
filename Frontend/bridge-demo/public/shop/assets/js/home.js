...
(function () {
  ...
  function pickImg(q) {
    ...
    return "assets/img/szenen/" + q[2] + ".webp";
  }
  function initQuick() {
  ...
  /* Das Altsystem kennt keine Auszeichnung "Topseller". Gezeigt werden darum
     Artikel, die es dort wirklich zu kaufen gibt: mit Bild und mit Preis. */
  function initTopseller() {
    var el = document.querySelector("[data-topseller]");
    if (!el) return;
    var list = C.products.filter(function (p) {
      return p.image && p.modus === "kauf" && p.price > 0;
    }).slice(0, 10);
    if (list.length < 6) {
      list = list.concat(C.products.filter(function (p) {
        return p.image && list.indexOf(p) < 0;
      })).slice(0, 10);
    }
    el.innerHTML = list.length
      ? list.map(MF.card).join("")
      : '<div class="bridge-load" style="min-width:100%">Der Live-Katalog wird gerade aus ' +
        "matten.de aufgebaut. Bitte in einem Moment neu laden.</div>";
  ...
  function initProjects() {
    ...
    el.innerHTML = PROJEKTE.map(function (p) {
      return '<a class="mag reveal" href="' + p[3] + '">' +
        '<img src="assets/img/szenen/' + p[0] + '.webp" alt="" loading="lazy">' +
  ...
  function initMagazin() {
    ...
    el.innerHTML = MAGAZIN.map(function (m) {
      return '<a class="mag reveal" href="' + m[3] + '">' +
        '<img src="assets/img/szenen/' + m[4] + '.webp" alt="" loading="lazy">' +
