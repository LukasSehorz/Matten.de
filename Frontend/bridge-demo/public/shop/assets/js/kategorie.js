/* =============================================================================
   Kategorie-/Listing-Seite — BRÜCKEN-FASSUNG

   Die Liste arbeitet auf window.CATALOG. Der kommt in dieser Fassung nicht
   aus einer statischen Datei, sondern wird vom Proxy live aus matten.de
   gebaut (GET /shop/assets/js/catalog.js). Die Filter greifen deshalb auf
   Felder zu, die es im Altsystem wirklich gibt — Kaufartikel gegen
   Anfrageartikel, freie Maße, Bild vorhanden. Erfundene Merkmale wie
   "Topseller" gibt es hier nicht mehr.
   ========================================================================== */
(function () {
  "use strict";
  var C = window.CATALOG || { categories: [], products: [] };
  var META = window.CATALOG_META || {};
  var PAGE = 24;
  ...
  function filtered() {
    ...
    if (state.price) {
      list = list.filter(function (p) {
        return p.price > 0 &&
          p.price >= state.price[0] && (state.price[1] === null || p.price < state.price[1]);
      });
    }
    if (state.only.has("massanfertigung")) {
      list = list.filter(function (p) { return !!p.custom; });
    }
    if (state.only.has("kauf")) list = list.filter(function (p) { return p.modus === "kauf"; });
    if (state.only.has("anfrage")) list = list.filter(function (p) { return p.modus === "anfrage"; });
    if (state.only.has("bild")) list = list.filter(function (p) { return !!p.image; });

    /* Artikel ohne Listenpreis stehen bei Preissortierung hinten — sie sind
       keine "0,00 €"-Artikel, sondern Anfrageartikel des Altsystems. */
    var ohne = function (p) { return !p.price; };
    if (state.sort === "price-asc") {
      list.sort(function (a, b) { return (ohne(a) - ohne(b)) || (a.price - b.price); });
    } else if (state.sort === "price-desc") {
      list.sort(function (a, b) { return (ohne(a) - ohne(b)) || (b.price - a.price); });
    } else if (state.sort === "name") {
      list.sort(function (a, b) { return String(a.name).localeCompare(String(b.name), "de"); });
    } else {
      list.sort(function (a, b) {
        var s = function (p) {
          return (p.image ? 40 : 0) + (p.price > 0 ? 20 : 0) +
                 (p.teaser ? 8 : 0) + Math.min(10, (p.colors || []).length);
        };
        return s(b) - s(a);
      });
    }
  ...
  function filters() {
    ...
    out += '<div class="filter-group"><h4>Preis</h4>' +
      PRICE_BANDS.map(function (b, i) {
        var n = list.filter(function (p) {
          return p.price > 0 &&
            p.price >= b[1][0] && (b[1][1] === null || p.price < b[1][1]);
        }).length;
        if (!n) return "";
    ...
    out += '<div class="filter-group"><h4>Eigenschaften</h4>' +
      [["kauf", "Direkt bestellbar (mit Preis)"],
       ["anfrage", "Anfrageartikel"],
       ["massanfertigung", "Freie Maßeingabe möglich"],
    ...
    out += '<div class="filter-group" style="border-bottom:0">' +
      ...
      '<p class="bridge-source" style="margin-top:18px">Alle Filter arbeiten auf Feldern, die ' +
      "aus matten.de gelesen sind. " +
      (META.stufe === 1
        ? "<b>Der Katalog ist noch in Stufe 1</b> — Preise und Varianten werden gerade im " +
          "Hintergrund nachgeladen. Seite in einer Minute neu laden."
        : "Katalogstand: " + MF.esc(String(META.stand || "unbekannt")).slice(0, 16).replace("T", " ") +
          " Uhr, " + (META.anzahlProdukte || 0) + " Artikel.") +
      "</p>" +
      '<p style="font-size:13.5px;color:var(--c-ink-muted);margin:14px 0 0">Nichts Passendes gefunden? ' +
