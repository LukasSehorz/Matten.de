/* =============================================================================
   Mattenfuchs-Shop — gemeinsame Logik  ·  BRÜCKEN-FASSUNG
   Kopf, Navigation, Suche, Warenkorb, Produktkarten, kleine Interaktionen.

   Unterschied zur Design-Fassung unter Frontend/shop/:
   Der Warenkorb liegt NICHT mehr im localStorage, sondern im Altsystem
   matten.de. Jede Warenkorb-Aktion geht über den lokalen Proxy an den alten
   Shop, und jeder Betrag auf dieser Seite ist eine Zahl, die von dort kommt —
   hier wird kein Geld gerechnet.
   ========================================================================== */
(function () {
  "use strict";
  ...
  /* ---------- Brücke zum Altsystem --------------------------------------- */
  /* Ein einziger Weg nach draußen: der lokale Proxy. Keine Adresse in diesem
     Frontend zeigt direkt auf matten.de — auch Produktbilder nicht, die
     laufen über /api/img/. */
  MF.api = function (pfad, opts) {
    opts = opts || {};
    ...
    return fetch(pfad, init).then(function (r) {
      return r.text().then(function (t) {
        var j;
        try { j = JSON.parse(t); }
        catch (e) { j = { ok: false, fehler: "Der Proxy hat kein JSON geliefert (HTTP " + r.status + ")." }; }
        j.httpStatus = r.status;
        return j;
      });
    }).catch(function (e) {
      return { ok: false, fehler: "Der Proxy ist nicht erreichbar: " + e.message, httpStatus: 0 };
    });
  };

  /* Kurzer, einheitlicher Fehlerkasten für Brücken-Fehler. */
  MF.apiFehler = function (j, was) {
    return '<div class="bridge-error"><b>' + esc(was) + " nicht möglich</b>" +
      "<span>" + esc((j && (j.fehler || j.grund)) || "Unbekannter Fehler des Altsystems.") + "</span></div>";
  };

  /* Der Hinweisstreifen, den jede umgestellte Seite trägt. */
  MF.bridgeNote = function () {
    return '<div class="bridgebar"><div class="container">' +
      MF.icon("info", "ico--sm") +
      "<p><b>Brücken-Demo</b> — dieses Frontend zeigt <b>echte Daten aus dem Altsystem " +
      "matten.de</b>. Katalog, Preise, Warenkorb und Adressdaten laufen live über einen " +
      "lokalen Proxy; Produktbilder kommen über <code>/api/img/</code>, nie direkt von " +
      "matten.de.</p>" +
      '<a href="/">Technische Demo-Seiten</a>' +
      "</div></div>";
  };

  /* Adresse der Produktseite. Der Slug ist der stabile Schlüssel im Katalog,
     der Live-Pfad geht als zweiter Parameter mit — damit die Produktseite das
     Altsystem auch dann fragen kann, wenn der Katalog gerade neu gebaut wird. */
  MF.prodHref = function (p) {
    return "produkt.html?p=" + encodeURIComponent(p.slug) +
      (p.pfad ? "&pfad=" + encodeURIComponent(p.pfad) : "");
  };

  /* Preisdarstellung einer Produktkarte. Es wird NICHT gerechnet: entweder es
     gibt eine Zahl vom Altsystem oder es steht ehrlich "auf Anfrage". */
  MF.preisHtml = function (p) {
    if (p.modus === "anfrage") {
      return '<div class="price price--quote">Preis auf Anfrage' +
        "<small>Anfrageartikel des Altsystems</small></div>";
    }
    if (!p.price) {
      return '<div class="price price--quote">' +
        (p.details ? "Preis auf Anfrage" : "Preis wird geladen …") +
        "<small>" + (p.details ? "kein Listenpreis im Altsystem" : "Artikelseite wird gerade gelesen") +
        "</small></div>";
    }
    return '<div class="price">' + (p.from ? '<span class="price__from">ab </span>' : "") +
      MF.eur(p.price) + "<small>Preis des Altsystems · zzgl. Versand</small></div>";
  };
  ...
  /* ---------- Katalogstand ------------------------------------------------ */
  /*
   * Wie alt sind die Produktdaten dieser Seite?
   *
   * Beim lokalen Server ist die Antwort "gerade eben" — er baut den Katalog
   * live. Auf Netlify kann er das nicht: der Aufbau kostet rund 390 Anfragen
   * an matten.de und etwa drei Minuten, eine Netlify-Function darf zehn
   * Sekunden laufen. Dort ist der Katalog deshalb eine beim Deployment
   * erzeugte Datei — und dann muss sichtbar sein, von wann sie stammt.
   *
   * Preise im Warenkorb und in der Kasse sind davon nicht betroffen: die
   * kommen weiterhin bei jedem Aufruf frisch aus dem Altsystem.
   */
  MF.katalogStand = function () {
    var m = window.CATALOG_META || {};
    if (!m.standText) return "";
    var vorgebaut = m.erzeugung === "build";
    return '<div class="footer__ai" style="margin-top:4px">' +
      "Produktdaten: Stand " + esc(m.standText) +
      (vorgebaut
        ? " (beim Deployment aus matten.de gelesen). Preise im Warenkorb und in der Kasse sind live."
        : " (live aus matten.de gelesen).") +
      "</div>";
  };
    ...
    return '' +
      '<footer class="footer"><div class="container">' +
        ...
        '<div class="footer__pay">' +
          ["Vorkasse", "Rechnung"].map(function (p) {
            return '<span class="paychip">' + p + "</span>";
          }).join("") +
          '<span style="margin-left:auto;font-size:13px;opacity:.75">Das Altsystem bietet auch ' +
          "PayPal und Kreditkarte an. Beide führen zum Zahlungsdienstleister Worldline/Ogone " +
          "und sind in dieser Brücke serverseitig gesperrt.</span>" +
        "</div>" +
  ...
  /* ---------- Warenkorb — liegt im Altsystem ------------------------------ */
  /*
   * Kein localStorage mehr. Der Warenkorb dieser Seite IST der Warenkorb der
   * PHP-Sitzung von matten.de; der Proxy hält sie pro Browser-Besucher.
   * Deshalb gibt es hier keine Summenrechnung: Zwischensumme, Versand,
   * Umsatzsteuer und Gesamtbetrag kommen als fertige Texte von dort.
   */
  MF.cart = {
    state: null,          // letzte Antwort von /api/cart
    geladen: false,

    uebernehmen: function (j) {
      if (j && j.ok) {
        MF.cart.state = j;
        MF.cart.geladen = true;
      }
      MF.cart.paint();
      document.dispatchEvent(new CustomEvent("cart:change", { detail: j }));
      return j;
    },

    laden: function () {
      return MF.api("/api/cart").then(MF.cart.uebernehmen);
    },

    /* payload: { pfad, anzahl, werte:{ "attribute[…]": wert }, kommentar } */
    add: function (payload) {
      return MF.api("/api/cart/add", { method: "POST", body: payload }).then(MF.cart.uebernehmen);
    },

    setQty: function (key, anzahl) {
      return MF.api("/api/cart/menge", { method: "POST", body: { key: key, anzahl: anzahl } })
        .then(MF.cart.uebernehmen);
    },

    remove: function (key) { return MF.cart.setQty(key, 0); },

    clear: function () {
      return MF.api("/api/cart/clear", { method: "POST", body: {} }).then(MF.cart.uebernehmen);
    },

    items: function () { return (MF.cart.state && MF.cart.state.items) || []; },
    count: function () { return (MF.cart.state && MF.cart.state.count) || 0; },
    ...
    paint: function () {
      ...
      document.querySelectorAll("[data-cart-total]").forEach(function (el) {
        /* Betrag als Text des Altsystems, ungerechnet. */
        el.textContent = s && s.gesamt ? s.gesamt : (MF.cart.geladen ? "0,00 €" : "…");
      });
    }
  };
  ...
  /* ---------- Produktkarte ------------------------------------------------ */
  MF.card = function (p) {
    var href = MF.prodHref(p);
    /* Der Shop kennt keine Auszeichnungen wie "Topseller". Die einzige echte
       Unterscheidung ist Kaufartikel gegen Anfrageartikel — die zeigen wir. */
    var badge = p.modus === "anfrage"
      ? '<div class="card__badges"><span class="badge badge--custom">Anfrage</span></div>'
      : "";
    /* Farbmuster-Bilder gibt es live nicht; statt leerer Kreise die Zahl. */
    var farben = (p.colors && p.colors.length)
      ? '<p class="card__colors">' + p.colors.length + " Farben zur Auswahl</p>"
      : "";
    return '' +
  ...
  MF.mount = function () {
    ...
    if (h) h.innerHTML = MF.headerHTML() + MF.bridgeNote();
